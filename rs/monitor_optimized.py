...
TELE = Path(__file__).parent

def load_config():
    return json.loads((TELE / 'config.json').read_text(encoding='utf-8'))
...
def run_once(config):
    ...
    storage.init_db()
    all_notifications = HRSSSpider().run()

    keywords = config.get("keywords", [])
    print(f"\n[筛选] 关键词匹配: {len(filter_notifications(all_notifications, keywords))} 条")

    new_notifications = []
    for item in filter_notifications(all_notifications, keywords):
        if not storage.is_exists(item["id"]):
            storage.save_notification(item["id"], item["title"], item["url"], item["date"], item["source"], matched=True)
